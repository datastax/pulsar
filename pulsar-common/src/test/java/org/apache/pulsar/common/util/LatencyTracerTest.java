...
public class LatencyTracerTest {
    ...
    private static LatencyTracer.NanoTimeSupplier testNanoTimeSupplier(long... nanoTimes) {
        return new LatencyTracer.NanoTimeSupplier() {
            ...
            public long getNanos() {
                final Long nanos = nanoTimesQueue.poll();
                assertNotNull(nanos);
                return nanos;
            }
        };
    ...
    public void testMulti() {
        LatencyTracer tracer = new LatencyTracer(new LinkedList<>(), testNanoTimeSupplier(10_000_000L, 30_000_000L,
                70_000_000L, 80_000_000L));
        tracer.trace("A");
        tracer.trace("B");
        LatencyTracer.Snapshot snapshot = tracer.getLatency();
    ...
    public void testEmpty() {
        LatencyTracer tracer = new LatencyTracer(new LinkedList<>(), testNanoTimeSupplier(0L, 20_000_000L));
        LatencyTracer.Snapshot snapshot = tracer.getLatency();
    ...
    public void testZeroMs() {
        LatencyTracer tracer = new LatencyTracer(new LinkedList<>(), testNanoTimeSupplier(0L, 999_999L, 2_000_000L,
                2_100_000L));
        tracer.trace("A");
        tracer.trace("B");
        tracer.trace("C");
    ...
    public void testTraceFuture() throws Exception {
        LatencyTracer tracer = new LatencyTracer(new LinkedList<>(), System::nanoTime);
        final CompletableFuture<Integer> future = CompletableFuture.completedFuture(100);
        assertSame(tracer.trace("A", future), future);
        final String latency = tracer.getLatency().description();
        assertTrue(Pattern.compile("total: \\d+ ms").matcher(latency).matches(), latency);

        final CompletableFuture<Integer> future2 = new CompletableFuture<Integer>();
        delayedExecutor(500, TimeUnit.MILLISECONDS).execute(() -> future2.complete(1));
        final CompletableFuture<Integer> tracedFuture = tracer.trace("B", future2);
        assertNotSame(tracedFuture, future2);
        assertEquals(tracedFuture.get(), 1);
        final LatencyTracer.Snapshot snapshot = tracer.getLatency();
    ...
    public void testTraceFailedFuture() throws Exception {
        final LatencyTracer tracer = new LatencyTracer(new LinkedList<>(), testNanoTimeSupplier(1L));

        final CompletableFuture<Void> future = new CompletableFuture<Void>();
        delayedExecutor(100, TimeUnit.MILLISECONDS).execute(() -> future.completeExceptionally(
    ...
    // Replace it with `CompletableFuture.delayedExecutor` after upgraded to Java 9 or later
    private static Executor delayedExecutor(long delay, TimeUnit unit) {

        return command -> new Thread(() -> {
            try {
